...
def main(page: ft.Page):
    ...
    Rule 5: DO NOT use markdown, bold text, or asterisks like ** in your response.'''
    ...
    def listen_audio(e):
        user_input.hint_text = 'Recording for 5 seconds... Speak now!'
        page.update()
        
        try:
            # Record using sounddevice (Bypasses PyAudio)
            fs = 44100
            seconds = 5
            recording = sd.rec(int(seconds * fs), samplerate=fs, channels=1)
            sd.wait()
            sf.write('temp_voice.wav', recording, fs)
            
            user_input.hint_text = 'Processing audio...'
            page.update()
            
            # Recognize audio from saved file
