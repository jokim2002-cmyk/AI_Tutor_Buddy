...
class Std7EnglishMediumV1SmokeTests(unittest.TestCase):
    @classmethod
    ...
    def test_std7_english_medium_four_subjects_repository_load(self) -> None:
        for subj_name in self.subjects:
            syllabus = self.repo.find(board="GSEB", medium="English", standard=7, subject=subj_name)
            self.assertIsNotNone(syllabus, f"Failed to load syllabus for Std 7 English-medium {subj_name}")
            self.assertGreater(len(syllabus.chapters), 0, f"Syllabus for {subj_name} has no chapters")

    def test_std7_english_medium_test_paper_generation_and_forbidden_phrases(self) -> None:
        for subj_name in self.subjects:
            syllabus = self.repo.find(board="GSEB", medium="English", standard=7, subject=subj_name)
            ...
            self.assertEqual(paper_ch.total_marks, 25, f"{subj_name} chapter test total marks must be 25")
            for phrase in self.forbidden_phrases:
                self.assertNotIn(
                    phrase.casefold(),
                    text_ch.casefold(),
                    f"{subj_name} chapter test paper contains forbidden generic phrase: '{phrase}'",
                )
            ...
            self.assertEqual(paper_full.total_marks, 100, f"{subj_name} full syllabus test total marks must be 100")
            for phrase in self.forbidden_phrases:
                self.assertNotIn(
                    phrase.casefold(),
                    text_full.casefold(),
                    f"{subj_name} full syllabus test paper contains forbidden generic phrase: '{phrase}'",
                )

    def test_std7_english_medium_answer_evaluation_policy(self) -> None:
        for subj_name in self.subjects:
            syllabus = self.repo.find(board="GSEB", medium="English", standard=7, subject=subj_name)
            self.assertIsNotNone(syllabus)
            ...
            # 1. 1-mark objective/factual question must evaluate to Correct or Incorrect
            ...
            score_wrong, status_wrong, _ = evaluate_single_test_answer(
                q_1m.question_text,
                "Random completely wrong answer statement.",
                q_1m.solution_guide,
                q_1m.max_marks,
            )
            self.assertEqual(score_wrong, 0.0)
            self.assertEqual(status_wrong, "Incorrect")
            ...
            self.assertGreater(len(desc_qs), 0, f"{subj_name} full syllabus paper must contain descriptive questions")
            ...
            self.assertIn(
                ...
                f"{subj_name} descriptive answer evaluation returned invalid status: {status_desc}",
            )
